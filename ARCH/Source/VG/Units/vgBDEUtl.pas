...
begin
  if Database is TvgDatabase then
    ...
    LockDatabaseProc(Database) else EnterCriticalSection(FDBLock);
end;
...
begin
  if Database is TvgDatabase then
    ...
    UnLockDatabaseProc(Database) else LeaveCriticalSection(FDBLock);
end;
...
begin
  ...
  GetMem(Data, SizeOf(PSetDBFlagData));
  try
...
var
  ...
  begin
    ...
    begin
      ...
      with Field do
      begin
        if (DataType in [ftBytes, ftVarBytes]) or {$IFDEF _D3_}IsBlob{$ELSE}InheritsFrom(TBlobField){$ENDIF}
...
initialization
  InitializeCriticalSection(FDBLock);
