...
uses TypInfo, SysUtils, DB, Forms, DBTables, vgVCLRes, vgTools, vgDB, vgDBUtl, vgDBCtrl,
  vgDBConv, vgBDECon, vgConvEd, vgBDE, vgDBTree, Explorer, ExplrDB, ExplrBDE
  {$IFDEF _D3_}, vgComObj {$ENDIF}
...
const
  Masks: array[TTableType] of string[5] = ('', '*.DB', '*.DBF', '*.TXT' {$IFDEF _D4_}, '*.DBF' {$ENDIF});
var
  Table: TTable;
begin
