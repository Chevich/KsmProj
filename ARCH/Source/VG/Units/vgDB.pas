...
interface
uses Classes, vgTools, DB, {$IFDEF _D3_}DBCtrls, vgI0_TLB, ComObj, vgScript{$ELSE}DBTables{$ENDIF};
...
{ TUpdateObject }
  ...
  published
    property DeleteMacros: TDBParams index 5 read GetParamsIndex write SetParamsIndex {$IFDEF _D4_} stored False {$ENDIF};
    property DeleteParams: TDBParams index 4 read GetParamsIndex write SetParamsIndex {$IFDEF _D4_} stored False {$ENDIF};
    property InsertMacros: TDBParams index 3 read GetParamsIndex write SetParamsIndex {$IFDEF _D4_} stored False {$ENDIF};
    property InsertParams: TDBParams index 2 read GetParamsIndex write SetParamsIndex {$IFDEF _D4_} stored False {$ENDIF};
    property ModifyMacros: TDBParams index 1 read GetParamsIndex write SetParamsIndex {$IFDEF _D4_} stored False {$ENDIF};
    property ModifyParams: TDBParams index 0 read GetParamsIndex write SetParamsIndex {$IFDEF _D4_} stored False {$ENDIF};
  end;
...
uses vgVCLRes, Consts, DBConsts, {$IFDEF _D3_}BDEConst,{$ENDIF}
  SysUtils , vgUtils, vgDBUtl, Graphics, Forms{$IFNDEF _D3_}, BDE {$ENDIF};
...
begin
  ...
  {$ENDIF}
    ...
    begin
      StartPos := CurPos;
      while (CurChar <> #0) and (Literal or not not NameDelimiter(CurChar, [])) do
      begin
        Inc(CurPos);
        {$IFDEF _D3_}
        while (CurPos^ in LeadBytes) do Inc(CurPos, 2);
...
begin
  case DataType of
    ...
    ftBCD:
        if
        {$IFDEF _D3_}
        TDataSetHack(DataSet).BCDToCurr(Buffer, Value)
        {$ELSE}
        BCDToCurr(pFMTBCD(Buffer)^, Value)
        {$ENDIF} then
          AsBCD := Value else
          AsBCD := 0;
...
{$IFDEF _D4_}
procedure TUpdateObject.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('Data', ReadData, WriteData, True);
end;

procedure TUpdateObject.ReadData(Reader: TReader);
var
  I: Integer;
begin
  for I := 0 to 5 do
  begin
    Reader.ReadValue;
    Reader.ReadCollection(GetParamsIndex(I));
  end;
end;

procedure TUpdateObject.WriteData(Writer: TWriter);
var
  I: Integer;
begin
  for I := 0 to 5 do Writer.WriteCollection(GetParamsIndex(I));
end;
{$ENDIF}
