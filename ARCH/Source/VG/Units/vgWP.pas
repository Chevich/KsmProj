  ...
  try
    ...
    begin
      ...
      begin
        ...
        { There is strange thing with memos -   }
        { inserting memo doesn't clear bookmark }
        if (Pos(#10, Text) > 0) then
        try
          FWordBasic.EditBookmark(Name := Bookmark.Name, Delete := 1);
        except end;
      end else
  ...
  begin
    ...
    try
      ...
      try
        ...
        try
          FWordBasic := GetActiveOleObject(OleClassName);
          FActiveOleObject := True;
        except end;
