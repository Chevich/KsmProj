...
begin
  ...
  Application.Title := 'Банковская информация';
    ...
    begin
      ...
      while (LoginCount<3) and (not DM1.EtvDBGKSM.Connected) do
        try
          Inc(LoginCount);
          DM1.EtvDBGKSM.Open;
          { Контроль на доступ юзера к задаче "Банк" }
          if LoginCount=1 then begin
            ...
            if GetFromSQLText(DM1.EtvDBGKSM.DataBaseName,
              ...
              ShowMessage('Уважаемый пользователь '+UserName+'!'+#13+
                'У вас нет доступа для работы с этой задачей.');
              //+#13+'Для обеспечения доступа к задаче обращайтесь в отдел ИАП.');
              //+#13+'Для обеспечения доступа к задаче обращайтесь в отдел ИАП.');
              Application.Terminate;
            end;
          end;
        except
          MessageDlg('Невозможно подключиться к серверу!'+#13#10+'Имя пользователя или пароль неправильны.',mtError,[mbOk],0);
        end;
    end;
    if not DM1.EtvDBGKSM.Connected then Application.Terminate;
    Dm1.SetOptions;
    FormBankLabel.IncL(2,'Загрузка');
    Application.CreateForm(TDMV, DMV);
    Application.CreateForm(TDM2, DM2);
    Application.CreateForm(TDMC, DMC);
    FormBankLabel.IncL(1,'Открытие таблиц');
