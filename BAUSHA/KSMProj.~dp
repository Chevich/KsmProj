Program KSMProj;
{ -DirShb=D:\B\Shb - запускать Run с этим параметром дл€ работы с локальной пакой шаблонов, при разработке новых шаблонов и тестировании }
Uses
  {VCLDB40;VCLDBX40;}
  {MsCheck,}
  Dialogs,
  ...
  MdAuto in '..\Auto\MdAuto.pas' {ModuleAuto: TDataModule},
  ...
  Auto in '..\Auto\Auto.pas' {FormAuto};
...
begin
  try
    ...
    Application.Title := '–еализаци€';

    Application.CreateForm(TModuleBase, ModuleBase);
  if Optimize then begin
      ShowMessage('After Create TModuleBase');
    ...
    if Optimize then begin
      ShowMessage('After Create TModuleGeography');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TModuleCommon');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TModuleProd');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TModuleOrgs');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TModuleContract');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TModulePays');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TModuleInvoice');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TModuleClientsAdd');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TModuleShop');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TMdMat');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    if Optimize then begin
      ShowMessage('After Create TModuleWorkers');
      OptimizeLoad;
    end;
    {Screen.Forms[0].Show;}
    ...
    { „тобы нормально работала функци€ Round при округлении чисел типа n.5 }
...
(*Application.ProcessMessages;*)
  //Set8087CW(Default8087CW); { Ёксперименты с точностью вычислени€ вещественных чисел }
  { —м. Ќеочевидные особенности вещественных чисел - http://www.delphikingdom.com/asp/viewitem.asp?catalogid=374}
  Application.Run;
end.
